...
import { PRECISION_BENCH_MEMBERS } from '../components/home/HomePrecisionBench'
...
const leaders = [
  ...PRECISION_BENCH_MEMBERS.map(({ name, role, bio, img }) => ({ name, role, bio, img })),
]

export default function AboutUsPage() {
  const teamMembers = leaders
        ...
        <MotionConfig reducedMotion="user">
          <div className="flex flex-col gap-24 pb-16 pt-[24px] lg:gap-[160px]">
            ...
            <section className="px-4 sm:px-8">
              ...
              >
                <motion.div className="flex min-w-0 flex-col gap-6" variants={staggerContainer(0.04, 0.1)}>
                  <motion.p variants={fadeUp(14)} className="text-[12px] font-normal uppercase leading-[1.33] tracking-[0.2em] text-[#AFA2FF]">
                    Our Mission
                  </motion.p>
                  <motion.h1
                    variants={fadeUp(22)}
                  ...
                  <motion.p
                    variants={fadeUp(18)}
                    className="max-w-[672px] text-[20px] font-normal leading-[1.33] text-[#ABABAB] sm:text-[24px] sm:leading-[1.33]"
                  >
            ...
            <section className="bg-[#131313] px-4 py-16 sm:px-8 lg:py-32">
              <motion.div
                className="mx-auto flex max-w-[1216px] flex-col gap-12 lg:flex-row lg:gap-20"
                variants={staggerContainer(0.1, 0.12)}
                initial="hidden"
                whileInView="visible"
                viewport={viewportOnce}
              >
                <motion.div className="shrink-0 lg:max-w-[280px]" variants={fadeUp(18)}>
                  <h2 className="text-[28px] font-bold leading-[1.11] tracking-[-0.025em] sm:text-[36px]">Our Story</h2>
                </motion.div>
                <motion.div className="flex min-w-0 flex-1 flex-col gap-12" variants={staggerContainer(0.08, 0.11)}>
                  <motion.p variants={fadeUp(18)} className="text-[20px] font-light leading-[1.375] text-white sm:text-[24px]">
                  ...
                  <motion.div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-8" variants={staggerContainer(0.06, 0.12)}>
                    <motion.div variants={fadeUp(20)} className="flex flex-col gap-4 rounded-md border border-[rgba(72,72,72,0.15)] p-8" {...hoverCardBorderGlow}>
                      <StoryIconRoots />
                  ...
                  <motion.p variants={fadeUp(16)} className="text-[18px] leading-[1.625] text-[#ABABAB]">
            ...
            <section className="px-4 sm:px-8">
              <motion.div
                className="mx-auto flex max-w-[1216px] flex-col gap-12 lg:gap-20"
                variants={staggerContainer(0.1, 0.14)}
                initial="hidden"
                whileInView="visible"
                viewport={viewportOnce}
              >
                <motion.div className="flex flex-col items-center gap-6 text-center" variants={fadeUp(18)}>
                ...
                <motion.div className="grid grid-cols-1 gap-12 md:grid-cols-3 md:gap-8 lg:gap-10" variants={staggerContainer(0.08, 0.12)}>
                  {philosophy.map((item) => (
                    <motion.div key={item.n} variants={fadeUp(20)} className="flex flex-col">
            ...
            <section className="px-4 sm:px-8">
              <motion.div
                className="mx-auto flex max-w-[1216px] flex-col gap-12 lg:gap-16"
                variants={staggerContainer(0.1, 0.12)}
                initial="hidden"
                whileInView="visible"
                viewport={viewportOnce}
                  ...
                  {teamMembers.map((l) => (
                    ...
                    >
                      <div className="overflow-hidden rounded bg-[#262626]">
                        <div className="aspect-[318/397.5] w-full overflow-hidden">
                          <motion.img src={l.img} alt={l.name} className="h-full w-full object-cover" {...hoverMediaKen} />
                        </div>
                      </div>
            ...
            <section className="px-4 sm:px-8">
              <motion.div
                className="relative mx-auto max-w-[1216px] overflow-hidden rounded-lg border border-[rgba(72,72,72,0.15)] bg-[rgba(38,38,38,0.4)] px-8 py-16 backdrop-blur-[20px] sm:px-12 sm:py-20 lg:px-24 lg:py-24"
              ...
              >
                ...
                <div className="relative flex flex-col items-center gap-8 text-center">
                  <motion.h2 variants={fadeUp(22)} className="max-w-[680px] text-[40px] font-bold leading-none tracking-[-0.05em] sm:text-[52px] lg:text-[60px]">
                    Ready to build the future?
                  </motion.h2>
                  <motion.p variants={fadeUp(16)} className="max-w-[672px] text-[18px] leading-[1.4] text-[#ABABAB] sm:text-[20px] sm:leading-[1.4]">
                    Let&apos;s discuss how our precision engineering can accelerate your AI roadmap.
                  </motion.p>
                  <motion.div variants={fadeUp(14)}>
                    <MotionLink to="/contact" className="inline-block rounded-md px-10 py-4 text-[18px] font-bold leading-[1.56] text-black" style={{ background: 'linear-gradient(174deg, #7459F7 0%, #AFA2FF 100%)' }} {...hoverPrimaryCta}>
                      Get in Touch
                    </MotionLink>
                  </motion.div>
                </div>
